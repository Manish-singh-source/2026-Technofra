<?php include 'header.php'; ?>
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-189WWHXLSS"></script>
<script>
  window.dataLayer = window.dataLayer || [];

  function gtag() {
    dataLayer.push(arguments);
  }
  gtag('js', new Date());

  gtag('config', 'G-189WWHXLSS');
</script>
<!-- MS Clarity -->
<script type="text/javascript">
  (function(c, l, a, r, i, t, y) {
    c[a] = c[a] || function() {
      (c[a].q = c[a].q || []).push(arguments)
    };
    t = l.createElement(r);
    t.async = 1;
    t.src = "https://www.clarity.ms/tag/" + i;
    y = l.getElementsByTagName(r)[0];
    y.parentNode.insertBefore(t, y);
  })(window, document, "clarity", "script", "mxzdn16ndk");
</script>
<!--END MS Clarity -->
<meta name="description"
  content="With our performance marketing services, we drive business growth. Expertise in SEM, PPC, social media ads, and more. Maximize ROI. Contact us now!">
<title>Link Promotions And Exhibits Case Study | Technofra</title>
<style>
  .pt-60 {
    padding-top: 60px;
  }

  .cover-image {
    display: flex;
    /* Optional: If you want to center content */
    justify-content: center;
    /* Optional: Horizontally center */
    align-items: center;
    /* Optional: Vertically center */
  }

  .cover-image img {
    max-width: 100%;
    height: auto;
    /* Ensures the image scales proportionally */
  }
</style>




<?php include 'navbar.php'; ?>

<section class="pb-60">
  <div class="cover-image">
    <img src="assets/image/casestudy/linkpro/linkpro01.webp" alt="Link Promotions case study">
  </div>
</section>

<section class="pb-60">
  <div class="container">
    <div class="row justify-content-between align-items-center">
      <div class="col-lg-5 col-md-5">
        <div class="feature-img-content">
          <div class="section-heading" data-aos="fade-up">
            <h2>Link Promotions and Exhibitions</h2>
            <p>
              Link Promotions and Exhibitions has a proven record of being one of the best and biggest players in this industry for over 20 years.
            </p>
            <a href="https://linkpromotions.co.in/" target="blank" class="btn btn-outline-info mt-20">Explore More</a>
          </div>
        </div>
      </div>
      <div class="col-lg-6 col-md-6">
        <img src="assets/image/casestudy/linkpro/linkpro02.webp" alt="Link Promotions case study" class="dp-r-10" data-aos="fade-left" data-aos-anchor="#example-anchor" data-aos-offset="500" data-aos-duration="500">
      </div>
    </div>
  </div>
</section>
<section class="pb-60">
  <div class="container">
    <div class="row justify-content-between align-items-center dpreverse">
      <div class="col-lg-6 col-md-6 mob-pt-20">
        <img src="assets/image/casestudy/linkpro/linkpro03.webp" alt="Link Promotions case study" class="dp-r-10" data-aos="fade-right" data-aos-offset="300" data-aos-easing="ease-in-sine">
      </div>
      <div class="col-lg-5 col-md-5">
        <div class="feature-img-content">
          <div class="section-heading" data-aos="fade-up">
            <h2> Our Role</h2>
            <p>

              UX/UI Design, Visual Design, 3D Animation, Motion Graphics, Icon Development, Front-End Development, CMS, SEO, WordPress Integration, Ongoing Maintenance, Copywriting, Editing.
            </p>
          </div>
        </div>
      </div>

    </div>
  </div>
</section>
<section class="pb-60">
  <div class="container">
    <div class="row justify-content-between align-items-center">
      <div class="col-lg-5 col-md-5">
        <div class="feature-img-content">
          <div class="section-heading" data-aos="fade-up">
            <h2> About Link Promotions and Exhibits</h2>
            <p>

              Promotions and exhibits are part of a dynamic
              vision to serve various B2B and B2C sectors for
              brand communication activities, custom-built
              exhibition stands, events, and launches.
            </p>
          </div>
        </div>
      </div>
      <div class="col-lg-6 col-md-6">
        <img src="assets/image/casestudy/linkpro/linkpro04.webp" alt="Link Promotions case study" class="dp-r-10" data-aos="fade-left" data-aos-anchor="#example-anchor" data-aos-offset="500" data-aos-duration="500">
      </div>
    </div>
  </div>
</section>
<section class="pb-60">
  <div class="container">

    <div class="row justify-content-between align-items-center">
      <div class="col-lg-5 ">
        <div class="section-heading text-center aos-init aos-animate" data-aos="fade-up">

          <h2>Identification Card</h2>
          <p>A fast, approachable, and accessible site that tripled engagement.</p>
        </div>
      </div>
      <div class="col-lg-6 ">
        <img src="assets/image/casestudy/linkpro/linkpro06.webp" alt="Link Promotions case study" class="dp-r-10" data-aos="fade-left" data-aos-anchor="#example-anchor" data-aos-offset="500" data-aos-duration="500">
      </div>

    </div>
  </div>
</section>
<section class="pb-60">
  <div class="container">
    <div class="row justify-content-between align-items-center">
      <div class="col-lg-6 ">
        <img src="assets/image/casestudy/linkpro/linkpro08.webp" alt="Link Promotions case study" class="dp-r-10" data-aos="fade-left" data-aos-anchor="#example-anchor" data-aos-offset="500" data-aos-duration="500">
      </div>
      <div class="col-lg-5 ">
        <div class="section-heading text-center aos-init aos-animate" data-aos="fade-up">
          <h2>Link Promotions</h2>
          <p>An in-house Design Facility with a core creative team,skilled supervisors and thorough technical guidance ensures that the stand is designed to create more visibility to visitors and depict a strong branding in turn.</p>
        </div>
      </div>
    </div>
  </div>
</section>
<section class="pb-60">
  <div class="container">
    <div class="row justify-content-between align-items-center">
      <div class="col-lg-10 col-md-10 ">
        <div class="section-heading">
          <h2> Responsive Design for All Devices</h2>
          <p>
            This section is designed to be responsive, ensuring optimal readability and layout across all devices, from mobile phones to desktops, with effortless accessibility for every user.</p>
        </div>
      </div>
      <div class="col-lg-12 col-md-12">
        <img src="assets/image/casestudy/linkpro/linkpro09.webp" alt="Link Promotions case study" class="dp-r-10" data-aos="fade-up" data-aos-anchor-placement="center-bottom">
      </div>
    </div>
  </div>
</section>
<section class="pb-60">
  <div class="container">
    <div class="row justify-content-between align-items-center">
      <div class="col-lg-12 col-md-12">
        <img src="assets/image/casestudy/linkpro/linkpro07.webp" alt="Link Promotions case study" class="dp-r-10" data-aos="fade-up" data-aos-anchor-placement="center-bottom">
      </div>
    </div>
  </div>
</section>



<?php include 'potfolio-slider.php'; ?>


<?php include 'footer.php'; ?>