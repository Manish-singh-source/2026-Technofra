(function($) {
    ...
    let pageSmoother = null;
  
    $documentOn.ready( function() {
      const sectionLinkSelector = '.main-menu a[href^="#"]:not([href="#"]), .mean-nav a[href^="#"]:not([href="#"]), a.theme-btn[href^="#"]:not([href="#"])';

      const getHeaderOffset = () => {
        const header = document.querySelector("#header-sticky");
        return header ? header.getBoundingClientRect().height + 18 : 100;
      };

      const setActiveNavLink = (targetHash) => {
        $(sectionLinkSelector).removeClass("is-active");
        $(sectionLinkSelector).filter(`[href="${targetHash}"]`).addClass("is-active");
      };
      ...
      const scrollToSection = (targetHash) => {
        ...
        if (pageSmoother) {
          const targetY = Math.max(pageSmoother.offset(target, "top top") - headerOffset, 0);
          pageSmoother.scrollTo(targetY, true);
    ...
    ================================ */
      ...
      $documentOn.on("click", sectionLinkSelector, function(e) {
        const targetHash = $(this).attr("href");

        if (!targetHash || !targetHash.startsWith("#") || !$(targetHash).length) {
          return;
        }
   ...
   if ($('.accordion-box').length) {
    ...
    if ($('.gt-project-area5').length > 0) {
		let project_text = gsap.timeline({
			scrollTrigger: {
				trigger: ".gt-project-area5",
				start: 'top center-=350',
				end: "bottom 50%",
				pin: ".gt-title5",
				markers: false,
		...
		project_text.set(".gt-title5", {
			scale: .6,
			duration: 2
		})
		project_text.to(".gt-title5", {
			scale: 1,
			duration: 2
		})
		project_text.to(".gt-title5", {
    ...
    const navSections = [...new Set($(sectionLinkSelector).map(function () {
        const hash = $(this).attr("href");
        return hash && $(hash).length ? hash : null;
    }).get().filter(Boolean))];

    if (navSections.length) {
        setActiveNavLink(navSections[0]);

        navSections.forEach((sectionHash, index) => {
            ScrollTrigger.create({
                trigger: sectionHash,
                start: () => `top top+=${getHeaderOffset() + 40}`,
                end: () => {
                    const nextSection = navSections[index + 1];
                    return nextSection ? `bottom top+=${getHeaderOffset() + 40}` : "bottom center";
                },
                onEnter: () => setActiveNavLink(sectionHash),
                onEnterBack: () => setActiveNavLink(sectionHash),
            });
        });
    }
