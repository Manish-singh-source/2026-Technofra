<?php include 'header.php'; ?>
<title>Rajprabha Case Study | Technofra</title>
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-189WWHXLSS"></script>
<script>
    window.dataLayer = window.dataLayer || [];

    function gtag() {
        dataLayer.push(arguments);
    }
    gtag('js', new Date());

    gtag('config', 'G-189WWHXLSS');
</script>
<style>
  .pt-60 {
    padding-top: 60px;
  }

  .cover-image {
    display: flex;
    /* Optional: If you want to center content */
    justify-content: center;
    /* Optional: Horizontally center */
    align-items: center;
    /* Optional: Vertically center */
  }

  .cover-image img {
    max-width: 100%;
    height: auto;
    /* Ensures the image scales proportionally */
  }
</style>




<?php include 'navbar.php'; ?>

<section class="pb-60">
  <div class="cover-image">
    <img src="assets/image/casestudy/rajprbha/rajprbha01.webp" alt="Rajprbha case study">
  </div>
</section>

<section class="counter-with-video pb-60">
  <div class="container">

    <div class="row justify-content-between align-items-center dpreverse">

      <div class="col-lg-5 col-md-5">
        <div class="feature-img-content">
          <div class="section-heading" data-aos="fade-up">
            <h2>Rajprabha</h2>
            <p>
            In the annals of altruism, the late Shri. Rajendra Kumar Agarwal stands as a shining exemplar. He wasn't just a founder; he was the benevolent maestro who crafted the blueprint for an exceptional real estate empire. Thus emerged the Agarwal Group of Companies, a legacy spanning six illustrious decades. 
            </p>
            <a href="https://rajprabha.co.in/" target="blank" class="btn btn-outline-info mt-20">Explore More</a>
          </div>

        </div>
      </div>
      <div class="col-lg-6 col-md-6">
        <img src="assets/image/casestudy/rajprbha/rajprbha02.webp" alt="Rajprbha case study" class="dp-r-10 sbox" data-aos="fade-left" data-aos-anchor="#example-anchor" data-aos-offset="500" data-aos-duration="500">
      </div>
    </div>
  </div>
</section>
<section class="counter-with-video pb-60">
  <div class="container">

    <div class="row justify-content-between align-items-center">
      <div class="col-lg-6 col-md-6">
        <img src="assets/image/casestudy/rajprbha/rajprbha03.webp" alt="Rajprbha case study" class="dp-r-10 sbox" data-aos="fade-right" data-aos-offset="300" data-aos-easing="ease-in-sine">
      </div>
      <div class="col-lg-5 col-md-5 mob-pt-20">
        <div class="feature-img-content">
          <div class="section-heading" data-aos="fade-up">
            <h2>About Rajprabha </h2>
            <p>

            Amidst the ever-growing dreams and ambitions of our nation, a visionary by the name of Shri. Rajendra Kumar Agarwal graced us with his quixotic spirit and boundless generosity. He breathed life into a real estate venture known as the Agarwal Group of Companies, not merely as a benevolent act but as a testament to his visionary, enterprising prowess.
            </p>
          </div>

        </div>
      </div>

    </div>
  </div>
</section>


<section class="pb-60">
  <div class="container">

    <div class="row align-items-center">
      <div class="col-lg-12 col-md-12">
        <img src="assets/image/casestudy/rajprbha/rajprbha04.webp" alt="Rajprbha case study" class="dp-r-10" data-aos="fade-up"  data-aos-anchor-placement="center-bottom">
      </div>

    </div>
  </div>
</section>

<section class="pb-60">
  <div class="container">

    <div class="row align-items-center">
      <div class="col-lg-12 col-md-12">
        <img src="assets/image/casestudy/rajprbha/rajprbha05.webp" alt="Rajprbha case study" class="dp-r-10" data-aos="fade-up"  data-aos-anchor-placement="center-bottom">
      </div>

    </div>
  </div>
</section>
<?php include 'potfolio-slider.php'; ?>
<?php include 'footer.php'; ?>